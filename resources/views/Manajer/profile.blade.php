    ...
    <div class="content">
      <div class="page-header">
        <div class="page-title">
          <h4>Profile Role Manajer PT Raja Perkasa</h4>
          <h6>
            @foreach($manajerUsers as $manajer)
              {{ $manajer->name }}@if(!$loop->last), @endif
            @endforeach
          </h6>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-body">
          <form action="{{ route('updatemanajerprofile', $user->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
