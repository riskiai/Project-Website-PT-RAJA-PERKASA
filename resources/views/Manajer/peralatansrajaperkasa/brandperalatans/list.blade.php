  ...
  <div class="content">
    <div class="page-header">
      <div class="page-title">
        <h4>List Data Brand Peralatan PT Raja Perkasa</h4>
      </div>
      <div class="page-btn">
        <a href="{{ route('brandperalatancreate') }}" class="btn btn-added">
          <img src="{{ asset('assets/img/icons/plus.svg') }}" class="me-2" alt="img" />
          Tambah Data Brand Peralatan
        </a>
      </div>
    </div>
        ...
        <div class="table-responsive">
          <table class="table datanew text-center">
            <thead>
              <tr>
                <th>No</th>
                <th>Nama Brand Peralatan</th>
                <th>Status Brand Peralatan</th>
                <th>Created Date</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ $item->nama_brand_peralatan }}</td>
                <td>
                  @if($item->status_brand_peralatans == 'active')
                ...
                <td>
                  <a class="me-3" href="{{ route('brandperalatanedit', $item->id) }}">
                    <img src="{{ asset('assets/img/icons/edit.svg') }}" alt="img" />
                  </a>
                  <button type="button" class="btn btn-link text-dark btn-delete" data-id="{{ $item->id }}" title="Menghapus Data">
                    <img src="{{ asset('assets/img/icons/delete.svg') }}" alt="img" />
                  </button>
                  <form id="deleteForm-{{ $item->id }}" action="{{ route('brandperalatandelete', $item->id) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                  </form>
                </td>
