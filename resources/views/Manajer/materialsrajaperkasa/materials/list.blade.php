...
<div class="page-wrapper">
  <div class="content">
    <div class="page-header">
      ...
      <div class="page-btn">
        <a href="{{ route('materialscreate') }}" class="btn btn-added">
          ...
          <table class="table datanew text-center">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  @if($item->status_materials == 'berfungsi')
                  <span class="badges bg-lightgreen">Berfungsi</span>
                  @else
                  <span class="badges bg-lightred">Tidak Berfungsi</span>
                  @endif
                </td>
                <td>{{ $item->created_at->format('Y-m-d') }}</td>
                <td>{{ $item->updated_at->format('Y-m-d') }}</td>
                <td>
                  <a class="me-3" href="{{ route('materialsedit', $item->id) }}">
                    <img src="{{ asset('assets/img/icons/edit.svg') }}" alt="img" />
                  </a>
                  <button type="button" class="btn btn-link text-dark btn-delete" data-id="{{ $item->id }}" title="Menghapus Data">
                    <img src="{{ asset('assets/img/icons/delete.svg') }}" alt="img" />
                  </button>
                  <form id="deleteForm-{{ $item->id }}" action="{{ route('materialsdelete', $item->id) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
